...
mu_ind=1
mu = np.array([mu_ind,mu_ind,mu_ind,mu_ind,mu_ind,mu_ind])

sigma_ind=.25
sigma = np.array([sigma_ind,sigma_ind,sigma_ind,sigma_ind,sigma_ind,sigma_ind])
...
@njit 
def DS(x):
    return np.exp(-(x-mu)/sigma)*S(x)**2/sigma
...
def WiCo_Jac(X,t,conn):
    ...
    diag = lambda i: -1 + (1-r[i]*X[i])*DSfloat(input_raw[i],i)*conn[i,i]-r[i]*Sfloat(input_raw[i],i)
    nodiag = lambda i,j: (1-r[i]*X[i])*DSfloat(input_raw[i],i)*conn[j,i]
...
def Kaplan_Yorke(raw_spec,tol=-1e-3):
