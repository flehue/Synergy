...
@njit#(float64[:](float64[:],float64[:],float64[:],float64[:],float64[:]),nopython=True)
def WiCo_stoch(x,t,sqdtD,a,b):
    conn=np.array([[16.,15.,5.8,0.2],[-12.,-3.,0.,0.],[a,b,16.,15.],[0.,0.,-12.,-3.]])
    Iinp=S(P + x@ conn)
    rand = np.zeros(N)
    for i in range(N):
        this = sqdtD*np.random.normal()
        rand[i]=this#np.zeros(4)
...
@njit
def run_stoch(a,b,X0,t,dt,SD_over_sqrtdt):
    time = np.arange(0,t,dt)
    x_t=np.zeros((len(time),N))
    x = X0
    for i,t in enumerate(time):
        x_t[i]=x
        x+=dt*WiCo_stoch(x,t,SD_over_sqrtdt,a,b)
    return x_t 
